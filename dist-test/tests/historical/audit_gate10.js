...
async function runAudit() {
    ...
    try {
        ...
        if (!log.find(t => t.from === LifecycleController_js_1.EngineState.INIT_ONCE && t.to === LifecycleController_js_1.EngineState.RUNNING)) {
        ...
        if (!snapshot)
            violations.push("Snapshot failed (returned null)");
        // Verify transitions: RUNNING -> SNAPSHOTTING -> RUNNING
        const snapEntry = log.find(t => t.to === LifecycleController_js_1.EngineState.SNAPSHOTTING && t.tick >= snapTick);
        const snapExit = log.find(t => t.from === LifecycleController_js_1.EngineState.SNAPSHOTTING && t.to === LifecycleController_js_1.EngineState.RUNNING && t.tick >= snapTick);
        if (!snapEntry)
            violations.push("Missing transition -> SNAPSHOTTING");
        if (!snapExit)
            violations.push("Missing transition SNAPSHOTTING -> RUNNING");
        // 8. Run 1 tick
        ...
        const restoreEntry = log.find(t => t.to === LifecycleController_js_1.EngineState.RESTORING);
        if (!restoreEntry)
            violations.push("Missing transition -> RESTORING");
        // 10. Run 1 tick
