...
async function verifyGate11Final() {
    ...
    if (distTestsExists)
        violations.push("dist/tests exists (should not)");
    if (distVerifyExists)
        violations.push("dist/verify exists (should not)");
    // 2. Import Isolation
    ...
    if (srcVerifyExists)
        violations.push("src/verify still exists (should be removed)");
    ...
    if (!testsEngineExists)
        violations.push("tests/engine missing");
    if (!testsHistoricalExists)
        violations.push("tests/historical missing");
    if (!docsExists)
        violations.push("docs/ missing");
