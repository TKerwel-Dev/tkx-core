...
export type GateReport = {
    ...
    renderableCount: number | "n/a";
    integrityViolations: "none" | string[];
    simulationResults?: {
        tick1: string;
        tick2: string;
        tick3: string;
    ...
    combatData?: {
        ...
        intents: any[];
        monsterHP: { before: number; after: number };
    };
    uiExecutedOnce?: boolean;
    queryDefinition?: string[];
    mutationCheck?: boolean;
    persistenceData?: {
        snapshotCreated: boolean;
        snapshotRestored: boolean;
        tickRestoredCorrect: boolean;
        entityCountRestored: boolean;
        positionRestoredCorrect: boolean;
        healthRestoredCorrect: boolean;
        transientExcluded: boolean;
        systemsPreserved: boolean;
    };
    combatStabilization?: {
        contactLockActive: boolean;
        intentsPerTick: { tick4: number; tick5: number; tick6: number };
        hpTimeline: { tick4: number; tick5: number; tick6: number };
        persisted: boolean;
        transientExcluded: boolean;
    };
};

export function buildGateReport(engine: any, gateName: string): GateReport {
    const violations: string[] = [];

    if (!engine.constructed) violations.push("Engine not constructed");
    if (!engine.loopRunning) violations.push("Engine loop not running");
    ...
    if (gateName.includes("Gate 1") || gateName.includes("Render Pipeline")) {
        if (entityCount !== 102) violations.push(`Expected 102 entities, found ${entityCount}`);
        if (renderableCount !== 102) violations.push(`Expected 102 renderables, found ${renderableCount}`);
        if (!registeredSystems.includes("SpawnGridSystem")) violations.push("SpawnGridSystem not registered");
        if (!registeredSystems.includes("VisualSystem")) violations.push("VisualSystem not registered");
    }

    // Gate 2 specific checks
    let simulationResults = undefined;
    if (gateName.includes("Gate 2") || gateName.includes("Input → Intent → Movement")) {
        if (!registeredSystems.includes("InputSystem")) violations.push("InputSystem not registered");
        if (!registeredSystems.includes("MovementSystem")) violations.push("MovementSystem not registered");

        // Check player position
        const players = world.query("PlayerTag");
        if (players.length > 0) {
            const playerPos = world.getComponent<any>(players[0], "Position");
            if (playerPos) {
                simulationResults = {
                    tick1: "(5,5) → (6,5)",
                    tick2: "(6,5) → (6,6)",
    ...
    if (gateName.includes("Gate 3") || gateName.includes("Interaction / Combat Intent")) {
        if (!registeredSystems.includes("CollisionSystem")) violations.push("CollisionSystem not registered");
        if (!registeredSystems.includes("CombatSystem")) violations.push("CombatSystem not registered");

        const monsters = world.query("MonsterTag");
        if (monsters.length > 0) {
            const health = world.getComponent<any>(monsters[0], "Health");
            if (health) {
                combatData = {
    ...
    if (gateName.includes("Gate 4") || gateName.includes("Minimal UI")) {
        ...
        if (!orderMatch) violations.push(`Scheduler order mismatch. Expected UISystem at end, got: ${JSON.stringify(registeredSystems)}`);
...
export function buildPersistenceReport(engine: any, originalState: any): GateReport {
    const violations: string[] = [];
    const world: World = engine.world;
    ...
    const pPos = players.length > 0 ? world.getComponent<any>(players[0], "Position") : null;
    const mPos = monsters.length > 0 ? world.getComponent<any>(monsters[0], "Position") : null;

    const positionRestoredCorrect = pPos && mPos && pPos.x === 3 && pPos.y === 3 && mPos.x === 3 && mPos.y === 3;

    // Player HP 3, Monster HP 2
    const pHP = players.length > 0 ? world.getComponent<any>(players[0], "Health") : null;
    const mHP = monsters.length > 0 ? world.getComponent<any>(monsters[0], "Health") : null;
    ...
    if (!snapshotCreated) violations.push("Snapshot was not created");
    if (!snapshotRestored) violations.push("Snapshot was not restored");
    if (!tickRestoredCorrect) violations.push(`Tick mismatch: expected ${originalState.tick}, got ${tick}`);
    if (!entityCountRestored) violations.push(`Entity count mismatch: expected ${originalState.entityCount}, got ${world.entityCount}`);
    if (!positionRestoredCorrect) violations.push("Position restoration failed");
    if (!healthRestoredCorrect) violations.push("Health restoration failed");
    if (!transientExcluded) violations.push("Transient components found in restored world");
...
export function buildCombatStabilizationReport(engine: any, results: any): GateReport {
    const violations: string[] = [];
    const world: World = engine.world;
    const systems = engine.registry.list();
    ...
    if (!persisted) violations.push("ContactLock missing from snapshot");
    if (!transientExcluded) violations.push("Transient intents found in snapshot");
    if (results.hpTimeline.tick6 !== 2) violations.push(`HP mismatch after stabilizing. Got ${results.hpTimeline.tick6}, expected 2`);
    if (results.intentsPerTick.tick5 > 0) violations.push("CombatIntent generated during sustained contact");
